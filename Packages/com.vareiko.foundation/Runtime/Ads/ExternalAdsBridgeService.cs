    ...
    public sealed class ExternalAdsBridgeService : IAdsService, VContainer.Unity.IInitializable
    {
        ...
        public async UniTask<AdLoadResult> LoadAsync(string placementId, CancellationToken cancellationToken = default)
        {
            ...
            if (result.Success)
            {
                ...
                _signalBus?.Publish(new AdLoadedSignal(result.PlacementId, result.PlacementType));
            }
            else
            {
        ...
        public async UniTask<AdShowResult> ShowAsync(string placementId, CancellationToken cancellationToken = default)
        {
            ...
            if (result.Success)
            {
                ...
                {
                    _signalBus?.Publish(new AdRewardGrantedSignal(result.PlacementId, result.RewardId, result.RewardAmount));
                }

                _signalBus?.Publish(new AdShownSignal(result.PlacementId, result.PlacementType, result.RewardGranted));
            }
            else
            {
